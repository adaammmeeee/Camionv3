...
entrepot *enchere_echange_fin(requete **rv, int nb_requete_vendre, int nb_entrepot, entrepot *a, int **graphe)
{
    for (int cpt_requete = 0; cpt_requete < nb_requete_vendre; cpt_requete++)
    {
        ...
        if (cpt_offre)
        {
            ...
            a[indice_e_offre_min].benefice_total -= faire_course(a[indice_e_offre_min].liste_camion[indice_c_offre_min], pos_camion, rv[cpt_requete]->origine, graphe, 0);
            a[indice_e_offre_min].benefice_total -= faire_course(a[indice_e_offre_min].liste_camion[indice_c_offre_min], rv[cpt_requete]->origine, rv[cpt_requete]->destination, graphe, 1);
        ...
        else
        {
            ...
            else if(camion_demande != -1 && cout_requete)
            {
                ...
                a[indice_e_demande].benefice_total -= faire_course(a[indice_e_demande].liste_camion[indice_c_demande], pos_camion, rv[cpt_requete]->origine, graphe, 0);
                a[indice_e_demande].benefice_total -= faire_course(a[indice_e_demande].liste_camion[indice_c_demande], rv[cpt_requete]->origine, rv[cpt_requete]->destination, graphe, 1);
                rv[cpt_requete]->a_vendre = 0;
...
int insertion_ensemble(requete **nouv, entrepot a, int nb_requete, requete *meilleure_requete, int *camion, int *new_trajet, int new_charge, int *new_taille_trajet, int **graphe)
{
    ...
    int cam = -1;
    int taille_trajet = 0;
    int *trajet = calloc(TAILLE_MAX_TRAJET, sizeof(int));
    int *charge = calloc(TAILLE_MAX_TRAJET - 1, sizeof(int));
    
    for(int i = 0; i < nb_requete; i++)
    {
        if(nouv[i])
        {
            int distance = insertion(*nouv, a, &cam, trajet, charge, &taille_trajet, 1, graphe);

            if(distance < distance_min)
            {
                meilleure_requete = *nouv;
